...
class DisplaytopsnptestrmController(BaseController):
	...
	def getCallMethodLsGivenType(type_id, extra_table_name=None):
		"""
		2010-2-24
			based on test_type.results_type, choose different ResultsClass & TestResultClass
		2009-2-22
			add argument extra_table_name
		2008-12-30
		"""
		ResultsClass, TestResultClass = model.Stock_250kDB.Stock_250kDB.getResultsAndTestResultsClass(top_snp_test_type_id=int(type_id))
		if not extra_table_name:
			extra_table_name = TestResultClass.table.name
		extra_tables = ' %s c '%extra_table_name
		extra_condition = 'c.results_id=s.id and c.type_id=%s'%type_id
		affiliated_table_name = ResultsClass.table.name
	...
	def getGeneListTypeLsGivenTypeAndPhenotypeMethodAndAnalysisMethod(type_id, call_method_id, phenotype_method_id, analysis_method_id, affiliated_table_name=None):
		...
		if not affiliated_table_name:
			affiliated_table_name = TestResultClass.table.name	#alias is 's'
		extra_tables = ' %s c '%ResultsClass.table.name
	...
	def generate2DPvalueForEnrichmentStatFigure(self, results_id=None, type_id=None, list_type_id=None, get_data_matrix=False):
		...
		if get_data_matrix:
			from_where_clause = "from %s t, %s y where t.type_id=y.id and t.results_id=%s and t.list_type_id=%s and y.id =%s"%\
				(TestResultClass.table.name, model.Stock_250kDB.CandidateGeneTopSNPTestRMType.table.name,\
		...
		#for the ratio (and/or pvalue) curve plot
		...
		if os.path.isfile(plot_file_path) and os.path.isfile(curve_file_path):
			return_data = PassingData(no_of_top_snps_info=no_of_top_snps_info, \
									min_distance_info=min_distance_info, rdata=rdata, \
									pvalue_png_fname=plot_fname,\
									curve_fname=curve_fname, curve_svg_fname='%s.svg'%curve_fname)
			return return_data
	...
	@classmethod
	def getImage(cls, id=None):
		"""
		2010-4-14
			becomes classmethod
			"id" could be a full-path filename.
		2008-10-26
			get image from a file(path=id), which is generated by showOne()
		"""
		response.headers['Content-type'] = 'image/png'
		if os.path.isfile(id):	# 2010-4-14 "id" is a full-path, rather than relative-path to the 'plots_store'
			plot_file_path = id
		else:
	...
	def showOne(self, id=None, type_id=None, list_type_id=None):
		...
		c.type_id = type_id
		ResultsClass, TestResultClass = model.Stock_250kDB.Stock_250kDB.getResultsAndTestResultsClass(top_snp_test_type_id=int(type_id))
		c.result = ResultsClass.get(results_id)
		...
		if return_data:
			...
			c.curve_fname = return_data.curve_fname
			c.curve_svg_fname = return_data.curve_svg_fname
			...
			c.curve_svg_fname = '%s.svg'%curve_fname
	...
	def queryImage(self, id=None, type_id=None, list_type_id=28, img_type='png_data'):
		...
		if results_id:
			...
			elif img_type=='png_data':
				return_data = self.generate2DPvalueForEnrichmentStatFigure(results_id=results_id, type_id=type_id, list_type_id=list_type_id)
				if return_data:
					return self.getImage(return_data.curve_fname)
					get_img_data_success = 1
			elif img_type=='svg_data':
				return_data = self.generate2DPvalueForEnrichmentStatFigure(results_id=results_id, type_id=type_id, list_type_id=list_type_id)
				if return_data:
					return self.getImage(return_data.curve_svg_fname)
					get_img_data_success = 1
			else:

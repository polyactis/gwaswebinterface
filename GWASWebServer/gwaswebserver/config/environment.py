...
def load_environment(global_conf, app_conf):
	...
	config.init_app(global_conf, app_conf, package='gwaswebserver',
	...
	if ('sql_echo' in config['app_conf'] and config['app_conf']['sql_echo'] == 'True'):
		 sql_echo = config['app_conf']['sql_echo']
	#model.setup()
	
	model.db = model.Stock_250kDB.Stock_250kDB(drivername=drivername, username=db_user, password=db_passwd, \
							hostname=hostname, database=dbname, schema=schema, pool_recycle=pool_recycle,sql_echo=sql_echo)
